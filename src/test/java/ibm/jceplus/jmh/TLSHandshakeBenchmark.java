...
public class TLSHandshakeBenchmark extends JMHBase {
    ...
    public static class ClientState {
        ...
        private void initializeClientContext() throws Exception {
            String keystorePath = "testkeys.p12";
            String keystorePassword = "password";
    ...
    @Benchmark
    public void testHandshake(ServerState server, ClientState client) throws Exception {
        SSLSocket clientSocket = null;
        try {
            // Create a new socket for each handshake
            clientSocket = (SSLSocket) client.clientFactory.createSocket("localhost", server.port);
            clientSocket.setEnabledProtocols(new String[]{"TLSv1.3"});
            clientSocket.setEnabledCipherSuites(new String[]{server.cipherSuite});
            ...
            if (server.payload > 0) {
                byte[] buffer = new byte[server.payload];
            ...
            if (server.payload > 0) {
                byte[] buffer = new byte[server.payload];
                int totalRead = 0;
                while (totalRead < server.payload) {
                    int read = clientSocket.getInputStream().read(buffer, totalRead, server.payload - totalRead);
                    if (read == -1) break;
                    totalRead += read;
                }
            } else {
