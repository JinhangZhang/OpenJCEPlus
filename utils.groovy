    ...
    dir("java") {
        ...
        if (JAVA_RELEASE.contains("na.artifactory.swg-devops.com")) {
            withCredentials([usernamePassword(credentialsId: '7c1c2c28-650f-49e0-afd1-ca6b60479546', passwordVariable: 'ARTIFACTORY_PASSWORD', usernameVariable: 'ARTIFACTORY_USERNAME')]) {
                sh "curl -u \$ARTIFACTORY_USERNAME:\$ARTIFACTORY_PASSWORD ${java_link} > java.${file_extension}"
            }
        } else {
       ...
       if (software == "windows") {
        ...
        // Check if folder is already named 'jdk' (Artifactory downloads) or needs renaming (API downloads)
        def folderCheck = sh (
